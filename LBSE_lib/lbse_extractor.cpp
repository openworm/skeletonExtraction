...
void lbse::Extractor::calculateSDFForMeshGraph(MeshGraph * pMesh, structure::t3DModel * pModel, float * sdfValuesNormalizedMG, CVector3 ** sdfHalfVectors, CVector3 * sdfHalfVectorsMG){
	...
	*(sdfHalfVectors) = new CVector3[size];
	...
	for (int i=0; i < size; i++){
		...
		normalsdebug.push_back(pModel->pObject[0].pNormals[i]);
		sdfMGHFdebug.push_back(sdfHalfVectorsMG[i]);
	}
	...
	for(int i = 0; i < pModel->numOfObjects; i++)
	{
		...
		for(int j = 0; j < pObject->numOfFaces; j++){
			for(int whichVertex = 0; whichVertex < 3; whichVertex++){
				...
				MathStructures::Vector4 transformed = original;// * pModel->rootTrans; // positions are already transformed in this version !!
