...
#include "pct_PointCloudTriangulation.h"
...
Array2D<bool> DeleunayTriangulator::computeLocalTriangulation(int i, PCTMeshGraph * pMesh, std::vector<std::set<int>> globalNeighbourhoods, float * nor, bool visualization){
	  ...
	  dt.getTrianglePointers(vAllDelaunayTriangles);
		...
		// Deleunay triangualtion of polygon using poly2tri
		/*
		int id = 0;
		std::vector<p2t::Point *> polyline;
		polyline.reserve(numPoints);
		for (int k = 0; k< numPoints; k++) {
			polyline.push_back(new p2t::Point(pointsInPlane[k].x, (pointsInPlane[k].y, id)));
			id++;
		}
		...
		for (int j = 0; j < triangles.size(); j++) {
			...
			k = 0;
			for (std::set<int>::iterator it=neighs.begin(); it!=neighs.end(); ++it){
				if (Distance(pointsInPlane[k], PCTCVector2(a.x,a.y)) < 0.00001){
					i0 = *it;
				}
				if (Distance(pointsInPlane[k], PCTCVector2(b.x,b.y)) < 0.00001){
					i1 = *it;
				}
				if (Distance(pointsInPlane[k], PCTCVector2(c.x,c.y)) < 0.00001){
					i2 = *it;
				}
				k++;
			}
