...
namespace viennacl
{
  namespace linalg
  {
    namespace detail
    {
      ...
      */
      ...
      std::vector<typename MatrixType::value_type> inplace_qr_ublas(MatrixType & A, std::size_t block_size = 32)
      {
        ...
        for (std::size_t j = 0; j < min(A.size1(), A.size2()); j += block_size)
        {
          std::size_t effective_block_size = min(min(A.size1(), A.size2()), j+block_size) - j;
    ...
    void recoverQ(MatrixType const & A, VectorType const & betas, MatrixType & Q, MatrixType & R)
    {
      ...
      //
      std::size_t i_max = min(R.size1(), R.size2());
      ...
      std::size_t j_max = min(A.size1(), A.size2());
    ...
    void inplace_qr_apply_trans_Q(MatrixType const & A, VectorType1 const & betas, VectorType2 & b)
    {
      ...
      //
      for (std::size_t col_index=0; col_index<min(A.size1(), A.size2()); ++col_index)
      {
