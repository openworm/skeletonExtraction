 ...
 * 
 * @note This method provides an interface for the needs of a specific application. You won't probably need it ever.
 */
	CLASS_DECLSPEC
...
#if 0
/** \brief Refinement to achieve a certain maximum aspect ratio (new and experimental!)
 * 
 * Creates a mesh inside the area given by a Zone2 object that keeps a certain maximum aspect ratio (largest/smallest
 * side of a triangle)
 * 
 * @param pZone is the zone whose triangles are refined
 * @param minEdgeLength is a lower threshold on the edge length. Triangles with smaller edges are not refined.
 * @param maxEdgeLength is the maximum side length of each triangle (use DBL_MAX to ignore this property)
 * @param maxAspect is the maximum aspect ratio (longest side/shortest side, must be >2.0). 
 * @param bAllowConstraintSplitting specifies if also constraint edges may be splitted     
 * 
 * @note Constraint edges may enforce small angles. This method is new and still experimental. Your
 * feedback is appreciated.
 */
	CLASS_DECLSPEC
	void refineToAspectRatio(Zone2* pZone,double minEdgeLength,double maxEdgeLength,double maxAspect,bool bAllowConstraintSplitting);	
#endif
...
* CIS_IGNORE_DELAUNAY: Do \e not subdivide to keep the empty circle property 
...
*/
	CLASS_DECLSPEC
	bool checkZoneQuality(Zone2* pZone,double minAngle,const std::string& name);	
