...
inline void releaseStructure(structure::t3DModel *pModel){
...
inline void createModelNeighbours(structure::t3DModel *pModel, float delta){
  /* 	int offset = 0;
	for (int i = 0; i < pModel->numOfObjects; i++){
		structure::t3DObject *pObject = &pModel->pObject[i];
...
//			   Application->ProcessMessages();
 			   ...
 			   for (int k = 0;  k< pModel->numOfObjects; k++){
					structure::t3DObject *pObject2 = &pModel->pObject[k];
...
inline void calculateVerticeCount(structure::t3DObject *pObject){
...
inline void calculateVerticeCount(structure::t3DModel *pModel){
	...
	#ifdef _LOG
		logg.log(LOG_LEVEL_C_PARAMS, "pocet vertexov ", vertexCount);
	#endif
}

//---------------------------------------------------------------------------

inline CVector3 getVertexWithID(structure::t3DModel *pModel, int id){
	for (int i = 0; i < pModel->numOfObjects; i++){
		structure::t3DObject *pObject = &pModel->pObject[i];
			for (int j = 0; j < pObject->numOfVerts; j++){
...
inline int  findClosestPointID(structure::t3DModel *pModel, CVector3 sourcePoint){
...
inline void dumpNeighbours(structure::t3DModel *pModel){
...
inline void dumpDistances(structure::t3DModel *pModel){

	for(int i = 0; i < pModel->numOfObjects; i++)
	{
		if(pModel->pObject.size() <= 0) break;
		structure::t3DObject *pObject = &pModel->pObject[i];
		//logg.log("Number of faces: ",System::Convert::ToString(pObject->numOfFaces));
		for(int j = 0; j < pObject->numOfFaces; j++)
		{
			for(int whichVertex = 0; whichVertex < 3; whichVertex++)
			{
				...
				//logg.log((System::Convert::ToString(vertID)+"  "),System::Convert::ToString(pModel->pDistVert[vertID]));
				}
			}
		}
	  //logg.log("Max distance  :  ", System::Convert::ToString(pModel->maxDistance));
}

//---------------------------------------------------------------------------

inline int getTopPointID(structure::t3DModel *pModel){
	float top = -FLT_MAX;
	int id = -1;
	for (int i = 0; i < pModel->numOfObjects; i++){
		structure::t3DObject *pObject = &pModel->pObject[i];
