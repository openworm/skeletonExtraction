...
bool CLoad3DS::Import3DS(structure::t3DModel *pModel, char *strFileName)
{
	m_CurrentChunk = new structure::tChunk;				// Initialize and allocate our current chunk
	m_TempChunk = new structure::tChunk;					// Initialize and allocate a temporary chunk

	char strMessage[255] = {0};
	
	actVertID = 0;

	g_modelMax = structure::BoundingBox();
	...
	if (m_CurrentChunk->ID != PRIMARY_3DS)
	{
	...
	ProcessNextChunk(pModel, m_CurrentChunk, &g_modelMax);

	// Generate texture coords, if they are not in file , because of normal maps
    GenerateTextureCoords(pModel);
	...
	pModel->numOfVertices = actVertID;
	pModel->modelbb = g_modelMax;

	return 0;
}

void CLoad3DS::GenerateTextureCoords(structure::t3DModel *pModel){
	...
	for (int i = 0; i < pModel->numOfObjects; i++){
		...
		if (!pObject->bHasTexture)
			num += pObject->numOfVerts;
	}

	int index = 0;
	for (int i = 0; i < pModel->numOfObjects; i++){
		structure::t3DObject *pObject = &pModel->pObject[i];
		// if hasnt texture, generate UV coords
		if (!pObject->bHasTexture){
			delete[](pObject->pTexVerts);
			pObject->pTexVerts = NULL;
				pObject->pTexVerts = new CVector2[pObject->numOfVerts * 2];
				for (int j = 0; j < pObject->numOfVerts; j++) {
					pObject->pTexVerts[j].x = 1.0f/(float)num * (float)index;
					pObject->pTexVerts[j].y = 1.0f/(float)num * (float)index;
                	index ++;
				}
		}
	}
...
void CLoad3DS::ProcessNextChunk(structure::t3DModel *pModel, structure::tChunk *pPreviousChunk, structure::BoundingBox  *pModelMax)
{
	structure::t3DObject newObject = {0};					// This is used to add to our object list
	structure::tMaterialInfo newTexture = {0};				// This is used to add to our material list
	unsigned int version = 0;					// This will hold the file version
	//int buffer[50000] = {0};					// This is used to read past unwanted data

	m_CurrentChunk = new structure::tChunk;				// Allocate a new chunk
	...
	while (pPreviousChunk->bytesRead < pPreviousChunk->length)
	{
		...
		case OBJECTINFO_3DS:						// This holds the version of the mesh
			...
			ProcessNextChunk(pModel, m_CurrentChunk, pModelMax);
			break;

		case MATERIAL_3DS:							// This holds the material information
		...
		case OBJECT_3DS:							// This holds the name of the object being read
			...
			memset(&(pModel->pObject[pModel->numOfObjects - 1]), 0, sizeof(structure::t3DObject));

			// Get the name of the object and store it, then add the read bytes to our byte counter.
			m_CurrentChunk->bytesRead += GetString(pModel->pObject[pModel->numOfObjects - 1].strName);

			(pModel->pObject[pModel->numOfObjects - 1]).bb = structure::BoundingBox();

			// Now proceed to read in the rest of the object information
			ProcessNextObjectChunk(pModel, &(pModel->pObject[pModel->numOfObjects - 1]), m_CurrentChunk, pModelMax);

			if ( (pModel->pObject[pModel->numOfObjects - 1]).bb.x_min < pModelMax->x_min) {
				pModelMax->x_min = (pModel->pObject[pModel->numOfObjects - 1]).bb.x_min;
				pModelMax->x_min_v = (pModel->pObject[pModel->numOfObjects - 1]).bb.x_min_v;
			}
			if ( (pModel->pObject[pModel->numOfObjects - 1]).bb.x_max > pModelMax->x_max) {
				pModelMax->x_max = (pModel->pObject[pModel->numOfObjects - 1]).bb.x_max;
				pModelMax->x_max_v = (pModel->pObject[pModel->numOfObjects - 1]).bb.x_max_v;
			}

			if ( (pModel->pObject[pModel->numOfObjects - 1]).bb.y_min < pModelMax->y_min) {
				pModelMax->y_min = (pModel->pObject[pModel->numOfObjects - 1]).bb.y_min;
				pModelMax->y_min_v = (pModel->pObject[pModel->numOfObjects - 1]).bb.y_min_v;
			}
			if ( (pModel->pObject[pModel->numOfObjects - 1]).bb.y_max > pModelMax->y_max) {
				pModelMax->y_max = (pModel->pObject[pModel->numOfObjects - 1]).bb.y_max;
				pModelMax->y_max_v = (pModel->pObject[pModel->numOfObjects - 1]).bb.y_max_v;
			}
			if ( (pModel->pObject[pModel->numOfObjects - 1]).bb.z_min < pModelMax->z_min) {
				pModelMax->z_min = (pModel->pObject[pModel->numOfObjects - 1]).bb.z_min;
				pModelMax->z_min_v = (pModel->pObject[pModel->numOfObjects - 1]).bb.z_min_v;
			}
			if ( (pModel->pObject[pModel->numOfObjects - 1]).bb.z_max > pModelMax->z_max) {
				pModelMax->z_max = (pModel->pObject[pModel->numOfObjects - 1]).bb.z_max;
				pModelMax->z_max_v = (pModel->pObject[pModel->numOfObjects - 1]).bb.z_max_v;
			}

			break;

		case EDITKEYFRAME_3DS:
...
void CLoad3DS::ProcessNextObjectChunk(structure::t3DModel *pModel, structure::t3DObject *pObject, structure::tChunk *pPreviousChunk, structure::BoundingBox *pModelMax)
{

	//int buffer[50000] = {0};					// This is used to read past unwanted data

	// Allocate a new chunk to work with
	m_CurrentChunk = new structure::tChunk;
	...
	while (pPreviousChunk->bytesRead < pPreviousChunk->length)
	{
		...
		switch (m_CurrentChunk->ID)
		{
		case OBJECT_MESH_3DS:					// This lets us know that we are reading a new object

			// We found a new object, so let's read in it's info using recursion
			ProcessNextObjectChunk(pModel, pObject, m_CurrentChunk, pModelMax);
			break;

		case OBJECT_VERTICES_3DS:				// This is the objects vertices
			ReadVertices(pObject, m_CurrentChunk, pModelMax);
			break;

		case OBJECT_FACES_3DS:					// This is the objects face information
			ReadVertexIndices(pObject, m_CurrentChunk);
			break;

		case OBJECT_MATERIAL_3DS:				// This holds the material name that the object has
		...
		case OBJECT_UV_3DS:						// This holds the UV texture coordinates for the object
...
void CLoad3DS::ProcessNextMaterialChunk(structure::t3DModel *pModel, structure::tChunk *pPreviousChunk)
{
	//int buffer[50000] = {0};					// This is used to read past unwanted data
	...
	while (pPreviousChunk->bytesRead < pPreviousChunk->length)
	{
		...
		switch (m_CurrentChunk->ID)
		{
		case MATNAME_3DS:							// This chunk holds the name of the material

			// Here we read in the material name
			m_CurrentChunk->bytesRead += fread(pModel->pMaterials[pModel->numOfMaterials - 1].strName, 1, m_CurrentChunk->length - m_CurrentChunk->bytesRead, m_FilePointer);
			break;

		case MATDIFFUSE_3DS:						// This holds the R G B color of our object
			ReadColorChunk(&(pModel->pMaterials[pModel->numOfMaterials - 1]), m_CurrentChunk);
			break;

		case MATMAP_3DS:							// This is the header for the texture info

			// Proceed to read in the material information
			ProcessNextMaterialChunk(pModel, m_CurrentChunk);
			break;

		case MATMAPFILE_3DS:						// This stores the file name of the material
...
void CLoad3DS::ReadChunk(structure::tChunk *pChunk)
{
...
void CLoad3DS::ReadColorChunk(structure::tMaterialInfo *pMaterial, structure::tChunk *pChunk)
{
...
void CLoad3DS::ReadVertexIndices(structure::t3DObject *pObject, structure::tChunk *pPreviousChunk)
{
	...
	pObject->pFaces =   new structure::tFace [pObject->numOfFaces];
	memset(pObject->pFaces, 0, sizeof(structure::tFace)  * pObject->numOfFaces);
...
void CLoad3DS::ReadUVCoordinates(structure::t3DObject *pObject, structure::tChunk *pPreviousChunk)
{
...
void CLoad3DS::ReadVertices(structure::t3DObject *pObject, structure::tChunk *pPreviousChunk, structure::BoundingBox *pModelMax)
{
	...
	pObject->pVertID = new UINT[pObject->numOfVerts];
	memset(pObject->pVertID, 0, sizeof(UINT) * pObject->numOfVerts);
	for (int i = 0; i < pObject->numOfVerts; i++) {
		pObject->pVertID[i] = actVertID;
		actVertID ++;
	}
	...
	for(int i = 0; i < pObject->numOfVerts; i++)
	{
	    ...
	    if ( pObject->pVerts[i].x < pObject->bb.x_min) {
			pObject->bb.x_min = pObject->pVerts[i].x;
			pObject->bb.x_min_v = pObject->pVerts[i];
		}
		if ( pObject->pVerts[i].x > pObject->bb.x_max) {
			pObject->bb.x_max = pObject->pVerts[i].x;
			pObject->bb.x_max_v = pObject->pVerts[i];
		}

		if ( pObject->pVerts[i].y < pObject->bb.y_min) {
			pObject->bb.y_min = pObject->pVerts[i].y;
			pObject->bb.y_min_v = pObject->pVerts[i];
		}
		if ( pObject->pVerts[i].y > pObject->bb.y_max) {
			pObject->bb.y_max = pObject->pVerts[i].y;
			pObject->bb.y_max_v = pObject->pVerts[i];
		}
		if ( pObject->pVerts[i].z < pObject->bb.z_min) {
			pObject->bb.z_min = pObject->pVerts[i].z;
			pObject->bb.z_min_v = pObject->pVerts[i];
		}
		if ( pObject->pVerts[i].z > pObject->bb.z_max) {
			pObject->bb.z_max = pObject->pVerts[i].z;
			pObject->bb.z_max_v = pObject->pVerts[i];
		}
...
void CLoad3DS::ReadObjectMaterial(structure::t3DModel *pModel, structure::t3DObject *pObject, structure::tChunk *pPreviousChunk)
{
	...
	for(int i = 0; i < pModel->numOfMaterials; i++)
	{
		...
		if(strcmp(strMaterial, pModel->pMaterials[i].strName) == 0)
		{
			...
			// read textured faces for each object
			int numOfFaces;

			pPreviousChunk->bytesRead += fread(&numOfFaces, 1, 4, m_FilePointer);
			//pModel->pMaterials[i].pTextredFaces = new int[numOfFaces];
			//pModel->pMaterials[i].numOfTextredFaces = numOfFaces;

			/*for(int j = 0; j < numOfFaces; j++){
				int faceId = -1;
					pPreviousChunk->bytesRead += fread(&faceId, 1, 2, m_FilePointer);
				if (faceId < numOfFaces && faceId >= 0){
					pObject->pFaces[faceId].materialID = i;
					pModel->pMaterials[i].pTextredFaces[j] = faceId;
				}
			}*/
...
void generateTangent(CVector3 &s_tan, CVector3 &t_tan, CVector3 v1, CVector3 v2, CVector2 st1, CVector2 st2){
		...
		s_tan = CVector3((t2 * x1 - t1 * x2) * r, (t2 * y1 - t1 * y2) * r,
			(t2 * z1 - t1 * z2) * r);
		t_tan = CVector3((s1 * x2 - s2 * x1) * r, (s1 * y2 - s2 * y1) * r,
			(s1 * z2 - s2 * z1) * r);

		//logg.log(0, "pTempTangents 1/R", s1 * t2 - s2 * t1);
		//logg.log(0, "pTempTangents R", r);

		//logg.log(0, "pTempTangents_s", s_tan);
		//logg.log(0, "pTempTangents_t", t_tan);
	}



void CLoad3DS::ComputeNormals(structure::t3DModel *pModel)
{
	...
	for(int index = 0; index < pModel->numOfObjects; index++)
	{
		...
		structure::t3DObject *pObject = &(pModel->pObject[index]);

		// Here we allocate all the memory we need to calculate the normals
		pObject->pFaceNormals	= new CVector3 [pObject->numOfFaces];
		CVector3 *pTempNormals	= new CVector3 [pObject->numOfFaces];
		pObject->pNormals		= new CVector3 [pObject->numOfVerts];

		// tangents
		CVector3 *pTempTangents_s	= new CVector3 [pObject->numOfFaces];
		CVector3 *pTempTangents_t	= new CVector3 [pObject->numOfFaces];
		...
		for(int i=0; i < pObject->numOfFaces; i++)
		{
			...
			pObject->pFaceNormals[i] = vNormal;						// Assign the normal to the list of normals


			generateTangent(pTempTangents_s[i], pTempTangents_t[i], vPoly[1] - vPoly[0], vPoly[2] - vPoly[0],
			pObject->pTexVerts[pObject->pFaces[i].vertIndex[1]] - pObject->pTexVerts[pObject->pFaces[i].vertIndex[0]]
			,pObject->pTexVerts[pObject->pFaces[i].vertIndex[2]] - pObject->pTexVerts[pObject->pFaces[i].vertIndex[0]]);

		}

		//////////////// Now Get The Vertex Normals /////////////////

		CVector3 vSum(0.0f, 0.0f, 0.0f);
		CVector3 tSum_s(0.0f, 0.0f, 0.0f);
		CVector3 tSum_t(0.0f, 0.0f, 0.0f);
		int shared=0;
		...
		for (int i = 0; i < pObject->numOfVerts; i++)			// Go through all of the vertices
		{
			...
			{												// Check if the vertex is shared by another face
					...
					pObject->pFaces[j].vertIndex[2] == i)
				{
					...
					tSum_s = tSum_s + pTempTangents_s[j];
					tSum_t = tSum_t + pTempTangents_t[j];
			...
			pObject->pTangents[i] = DivideVectorByScaler(tSum_s, float(-shared));
			// Normalize the normal for the final vertex normal
			pObject->pNormals[i] = Normalize(pObject->pNormals[i]);

			// Gram-Schmidt orthogonalize
			pObject->pTangents[i] = Normalize(pObject->pTangents[i] - pObject->pNormals[i] * Dot(pObject->pNormals[i], pObject->pTangents[i]));

			vSum = CVector3(0,0,0);									// Reset the sum
			tSum_s = CVector3(0,0,0);
			tSum_t = CVector3(0,0,0);
		...
		delete [] pTempNormals;
		pTempNormals = NULL;
		delete [] pTempTangents_s;
		pTempTangents_s = NULL;
		delete [] pTempTangents_t;
		pTempTangents_t = NULL;
	}
}
