...
namespace AssimpFileHandler
{
	...
	void CAssimp::SetScene(const struct aiScene *scena)
	{
		scene = scena;
	}
	void CAssimp::LoadData(LinkedList<Face>* fc, LinkedList<Vertex>* pts)
	{
		...
		fc->InsertToEnd(fck);*/
		aiMatrix4x4 id;
		RecursiveLoad(scene, scene->mRootNode, fc, pts, (float*)&id);
	}

	void CAssimp::RecursiveLoad (const struct aiScene *sc, const struct aiNode* nd, LinkedList<Face>* fc, LinkedList<Vertex>* pts, float * matrixFather)
	{
		...
		//glPushMatrix();
			...
			Mat4 mat = Mat4((float*)&matica) * Mat4(matrixFather);
			...
			{
				RecursiveLoad(sc, nd->mChildren[n], fc, pts, (float*)&matica);
			}
		//glPopMatrix();
	}
